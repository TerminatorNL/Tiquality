...
import static com.github.terminatornl.tiquality.Tiquality.SCHEDULER;
import static com.github.terminatornl.tiquality.TiqualityConfig.MAX_CLAIM_RADIUS;
import static com.github.terminatornl.tiquality.util.Utils.ONE_DECIMAL_FORMATTER;
    ...
    public static void execute(ICommandSender sender, String[] args, PermissionHolder holder) throws CommandException {
         ...
         */
            ...
            if (args.length > 1) {
                ...
                range = MAX_CLAIM_RADIUS;
            }
            if (sender instanceof EntityPlayer == false) {
                throw new CommandException("You must be a player to use this command.");
            }
            EntityPlayer player = (EntityPlayer) sender;
            BlockPos leastPos_pre = player.getPosition().add(range * -1, 0, range * -1);
            BlockPos mostPos_pre = player.getPosition().add(range, 0, range);

            BlockPos leastPos = new BlockPos(leastPos_pre.getX(), 0, leastPos_pre.getZ());
            BlockPos mostPos = new BlockPos(mostPos_pre.getX(), player.world.getHeight() - 1, mostPos_pre.getZ());
            ...
            List<PlayerTracker> affectedPlayerTrackers = new LinkedList<>();
            for (ChunkPos pos : affectedChunks) {
                ...
                if (tracker instanceof PlayerTracker == false) {
                    sender.sendMessage(new TextComponentString(TextFormatting.RED + "Sorry, there's already a tracker nearby that prevents you " + TextFormatting.RED + "from claiming here. (" + tracker.getInfo().getUnformattedText() + TextFormatting.RED + ")"));
                    if (holder.hasPermission(PermissionHolder.Permission.ADMIN)) {
                ...
                if (trackerProfile.equals(profile) == false) {
                    if (affectedPlayerTrackers.contains(playerTracker) == false) {
                        affectedPlayerTrackers.add(playerTracker);
                    }
                }
            }
            final World affectedWorld = player.world;
            if (affectedPlayerTrackers.size() > 0) {
                /* There's other trackers nearby, we need to ask permission first */
                PlayerTracker.ClaimOverrideRequester requester = new PlayerTracker.ClaimOverrideRequester() {

                    private final List<PlayerTracker> requests = new LinkedList<>(affectedPlayerTrackers);
                    ...
                    public void onDeny(PlayerTracker tracker) {
                        ...
                        sender.sendMessage(new TextComponentString(PREFIX + TextFormatting.RED + "Your claim override request was denied by: " + tracker.getOwner().getName()));
                        isDenied = true;
                    }
                    ...
                    public void onOfflineOwner(PlayerTracker tracker) {
                        ...
                        sender.sendMessage(new TextComponentString(PREFIX + TextFormatting.RED + tracker.getOwner().getName() + " is not online, and cannot verify your claim request. You have to wait until all owners are online."));
                        isDenied = true;
                    }

                    @Override
                    public void onAccept(PlayerTracker existingTracker) {
                        requests.remove(existingTracker);
                        if (isDenied) {
                            return;
                        }
                        sender.sendMessage(new TextComponentString(PREFIX + existingTracker.getOwner().getName() + " accepted your request."));
                        if (requests.size() == 0) {
                            sender.sendMessage(new TextComponentString(PREFIX + "Everyone accepted your request. Claiming..."));
                            Tracker tracker = PlayerTracker.getOrCreatePlayerTrackerByProfile((TiqualityWorld) affectedWorld, profile);
                            ...
                            for (PlayerTracker request : requests) {
                                sender.sendMessage(new TextComponentString(PREFIX + "Waiting for: " + request.getOwner().getName()));
                            }
                        }
                    }
                ...
                sender.sendMessage(new TextComponentString(PREFIX + TextFormatting.RED + "There already are trackers within this radius (" + range + " + " + 16 + "). Sending a request to the following owner(s): " + existingOwners));
                for (PlayerTracker affectedPlayerTracker : affectedPlayerTrackers) {
                    affectedPlayerTracker.requestClaimOverride(requester, affectedWorld, leastPos, mostPos);
                }
            } else {
                /* There's no other trackers nearby */
         ...
         */
            ...
            mostPos = new BlockPos(mostPos.getX(), player.world.getHeight() - 1, mostPos.getZ());
         ...
         */
            ...
            if (sender instanceof Entity) {
                ...
                String trackerTextFeet = trackerFeet == null ? TextFormatting.AQUA + "NOT TRACKED" : TextFormatting.WHITE + trackerFeet.getInfo().getText();
                if (isMarkedFeet) {
                ...
                trackerTextBelow = trackerBelow == null ? TextFormatting.AQUA + "NOT TRACKED" : TextFormatting.WHITE + trackerBelow.getInfo().getText();
                if (isMarkedBelow) {
